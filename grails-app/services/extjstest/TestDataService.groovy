			...
			(1..5).each{subNr->
				...
				(1..10).each{subSubNr->
					...
					(1..3).each{
						...
						Asset asset=Asset.build(division: subSubDivion, validFrom:now-10, validTo:now+10)
						(1..3).each{validNr->
							Worker worker=Worker.build(division: subSubDivion)
							Laborforce.build(worker: worker, division: subSubDivion, validFrom:now-validNr, validTo:now-(validNr+1), asset:asset)
						}
					}
				}
			}
